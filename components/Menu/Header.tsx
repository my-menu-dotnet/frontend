    ...
    <header className="w-full flex flex-col justify-center items-center">
      <div className="w-full h-full">
        <div className="relative w-full pt-[33%] md:pt-[10%]">
          <Image
            src={menu.company.header?.url || FoodDefault}
          ...
          <div>
            ...
            {menu.company.address && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex flex-row items-center gap-1 rounded-md text-sm mt-1"
              >
                <CiLocationOn size={16} color={color} />
                {menu.company.address.state} - {menu.company.address.city}
              </a>
            )}
          </div>
          <address className="flex gap-2 text-black w-full items-center justify-end">
            {menu.company.email && (
